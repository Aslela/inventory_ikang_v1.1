...
<?php $this->load->view('modal/modal_add_edit_subkategori')?>
<script type="text/javascript">
    ...
    $(function() {
        ...
        $("#add-btn").click(function(){
            $('#subkategori-form')[0].reset();
            $('#created').empty();
            $('#last_modified').empty();
            $('.modal-title').text("Add New Sub Kategori");
            $('#btn-update').hide();
            $('#btn-save').show();
...
<div class="content-container" >
    <div class="well">
        <div class="row">
            <div class="col-lg-8">
                <button type="button" class="btn btn-primary btn-xl" id="add-btn" data-toggle="modal" data-target="#subkategori-modal">
                    <span class="glyphicon glyphicon-plus"></span>&nbsp Add Sub Kategori
                </button>
            </div>
            <div class="col-lg-4" id="search-box">
                <?php echo form_open('subKategori/searchSubKategori'); ?>
        ...
        <tbody>
            ...
            ?>
                    <tr>
                        ...
                        <td align='center' class="nr"><?=$row['SubKategori_Name']?></td>
                        <td align='center'> 
                            <a class="edit-nav" href="#<?php echo $no?>">           
                                <button type="button" class="btn btn-primary btn-xs edit-btn" data-toggle="modal" data-target="#subkategori-modal">
                                <span class="glyphicon glyphicon-pencil"></span>&nbsp Edit</button>      
                            </a>    
                           	<a class="btn btn-danger btn-xs " href="<?php echo site_url('subkategori/deleteSubKategori/'.$row['SubKategori_ID'])?>" 
                               onclick="return confirm('Are you sure to delete <?=$row['SubKategori_Name']?> ?');">
                                <i class="glyphicon glyphicon-trash"></i>&nbsp Delete
                            </a>
                        </td>
                        
                        <td class="id-item" style="display: none;"><?=$row['SubKategori_ID']?></td>
